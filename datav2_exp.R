...
mdf <- wi %>% select(Incident.Number,Date1.of.Occurrence) %>% filter(Date1.of.Occurrence >= '2015-01-01' & Date1.of.Occurrence <= '2017-12-31') %>% group_by(Date1.of.Occurrence) %>% mutate(Year.of.Occurrence=year(Date1.of.Occurrence),Day.of.Year=yday(Date1.of.Occurrence),cn=n()) %>% arrange(Incident.Number)
mdf <- unique(mdf)
w2015 <- mdf %>% select(-Incident.Number) %>% filter(Year.of.Occurrence==2015)
w2016 <- mdf %>% select(-Incident.Number) %>% filter(Year.of.Occurrence==2016)
w2017 <- mdf %>% select(-Incident.Number) %>% filter(Year.of.Occurrence==2017)

lsize=1

  ggplot(mdf) +
  geom_line(aes(x=Day.of.Year,y=cn,color='2015'),w2015,na.rm=TRUE,size=lsize) +
  geom_line(aes(x=Day.of.Year,y=cn,color='2016'),w2016,na.rm=TRUE,size=lsize) +
  geom_line(aes(x=Day.of.Year,y=cn,color='2017'),w2017,na.rm=TRUE,size=lsize) +
