...
p1 <- ggplot() + labs(x='Longitude',y='Latitude',title='Incidents for 2015') + geom_point(aes(x=w2015$X.Coordinate,y=w2015$Y.Coordinate),color='red',na.rm=TRUE) +
   geom_density_2d() #+
#   geom_text(aes(x=specials$x,y=specials$y,label=specials$name),color='blue',na.rm=TRUE)
p2 <- ggplot() + labs(x='Longitude',y='Latitude',title='Incidents for 2016') + geom_point(aes(x=w2015$X.Coordinate,y=w2015$Y.Coordinate),color='blue',na.rm=TRUE) #+
#  geom_text(aes(x=specials$x,y=specials$y,label=specials$name),color='red',na.rm=TRUE)
  
p3 <- ggplot() + geom_point(aes(x=w2015$X.Coordinate,y=w2015$Y.Coordinate),color='green',na.rm=TRUE)
